...
const mouseDownedInput = ref<HTMLInputElement | HTMLTextAreaElement | null>()
...
const virtualElement = ref({
    ...
    getClientRects() {
        return childReacts.value[childReacts.value.length - 1]
    },
})
...
async function attachTooltip() {
    ...
    console.log(virtualElement.value)
    ...
    if (tooltip.value) {
        ...
        console.log('position', x, y, strategy)
...
function getSelectedTextBounding(input: HTMLInputElement | HTMLTextAreaElement, start: number, end: number): DOMRect {
    ...
    console.log(111 clonedInput);
    

    const range = setSelectionRange(clonedInput, start, end)
...
async function handleMouseupInput() {
...
async function handleMouseup() {
    ...
    if (range?.collapsed) {
        if (mouseDownedInput.value) {
            boundingClientRect.value = getSelectedTextBounding(mouseDownedInput.value, mouseDownedInput.value.selectionStart, mouseDownedInput.value.selectionEnd)
            mouseDownedInput.value.setSelectionRange(mouseDownedInput.value.selectionStart, mouseDownedInput.value.selectionEnd)
        }
        else {
            boundingClientRect.value = range.getBoundingClientRect()
        }
        mouseDownedInput.value = null
    }
    setTimeout(() => {
        if (status.value === 'tooltip')
        ...
        <div :class="$style.homeContent">
            <div :class="$style.homeContentParaPhrase">
                ...
                <div :class="$style.homeContentLanguageList">
                    <textarea ref="textareaRef" style="width: 100px; height: 100px;" type="text"
                        @mouseup.prevent="handleMouseupInput" />
                    ...
                    <div v-show="isCollapseLanguageList" :class="$style.homeExpandLanguages">
                        <div v-for="(expandItem, id) in languageList" v-show="expandItem !== 'All'" :key="id"
                            :class="id === activeItem && $style.homeExpandLanguagesActiveItem"
                            @click="handleActiveItem(id)">
                            <span>{{ expandItem }}</span>
                            <Icon v-show="id === activeItem" :class="$style.homeIcon"
                                icon="material-symbols:check-small-rounded" color="rgb(46, 125, 50)" />
                        </div>
                    </div>
                </div>
                <div :class="$style.homeTextAreaBox">
                        ...
                        <div :class="$style.homeTextAreaRightBox">
                            ...
                            <div v-else-if="status === 'popover'" ref="popover" :style="{
                                ...
                                <div :class="$style.homePopoverHeader">
                                    ...
                                    <Icon :class="$style.homeIcon" icon="material-symbols:close-rounded" color="#A9A9A9"
                                ...
                                <div :class="$style.homePopoverContent">
                                    <div :class="$style.homePopoverContentHeader">
                                        <div :class="$style.homePopoverContentRefresh" @click="getRepalceText">
                                            <Icon :class="$style.homePopoverIcon" icon="basil:refresh-solid"
                                                color="#A9A9A9" />
                                            <span>Refresh</span>
                                        </div>
                                        <div :class="$style.homePopoverContentRefresh">
                                            <Icon :class="$style.homePopoverIcon"
                    ...
                    laser pointer -->
                            ...
                            <div ref="area" :class="$style.homeTextAreaTag" contenteditable
                                placeholder="To rewrite text, enter or paste it here and press &quot;Paraphrase.&quot;"
                                @blur="handleMouseBlur" @mouseup.prevent="handleMouseup">
                                {{ result }}
                            </div>
                        </div>
                    </div>
                ...
                <div :class="$style.homeGetPremiumContent">
                    ...
                    <div :class="$style.homeGetPremiumBtnBox">
                        <button :class="$style.homeGetPremiumBtn">
                            <svg class="MuiSvgIcon-root MuiSvgIcon-fontSizeMedium css-10dohqv" focusable="false"
                                aria-hidden="true" viewBox="0 0 16 15" style="width:18px;height:20px;margin-right:8px">
                                <path
